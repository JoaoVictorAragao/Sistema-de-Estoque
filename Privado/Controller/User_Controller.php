...
require __DIR__ . '/../Services/User_Service.php';
...
if ($acao == 'login') {
    ...
    if (isset($_GET['login'])) {
        ...
        $conn = new Conexao();
        $userService = new UserService($conn, $user);
        $userResponse = $userService->Verifica_Usuario();
    } else {
        echo json_encode(['status' => 'erro GET bugado']);
    }
    if ($userResponse > 0) {
        echo json_encode(['status' => 'existe usuário cadastrado']);
    } else {
        echo json_encode(['status' => 'não existe usuário cadastrado']);
    }
